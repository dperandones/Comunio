...
class Comunio:
    ...
    def login(self):
        ...
        req = str(self.session.post('http://'+self.domain+'/login.phtml',headers=headers,data=payload).content)
        if req.find('puntos en proceso') >= 0 or req.find('points in process')>=0:
